...
# 1. 페이지 설정
st.set_page_config(page_title="TERA FC 매니저 V3.2", page_icon="⚽", layout="wide")
...
def load_data():
    try:
        # ttl=0으로 설정하여 커넥션 내부 캐시를 무시하고 실제 시트를 읽음
...
# 🔥 세션에 데이터를 저장하지 않고 호출할 때마다 캐시된 데이터를 직접 참조
df = load_data()

# 4. 사이드바
with st.sidebar:
    st.title("⚙️ 명단 관리")
    if st.button("🔄 시트 강제 동기화 (캐시 삭제)"):
        st.cache_data.clear()
        st.rerun()
...
st.caption("저장 후 새로고침 시에도 상태가 유지되도록 동기화 로직이 강화되었습니다.")
...
if not df.empty:
    ...
    # 저장 버튼 로직 보완
    if st.button("💾 현재 참석 상태를 시트에 영구 저장", type="primary"):
        with st.spinner("구글 시트에 동기화 중..."):
            ...
            # 🔥 [가장 중요] 업데이트 직후 캐시를 완전히 비우고 페이지를 새로고침함
            st.cache_data.clear()
            st.success("✅ 저장 완료! 이제 새로고침해도 이 상태가 유지됩니다.")
            st.rerun()
